...
const DEFAULT_MAX_TOTAL_SIZE = 15 * 1024 * 1024;
const DEFAULT_MAX_FILES = 5;
const DEFAULT_ACCEPT = '.jpg,.jpeg,.png,.doc,.docx,.pdf';

const cx = (...classes) => classes.filter(Boolean).join(' ');

const stripErrorPrefix = (message) => {
...
const validateFile = ({ file, accept, maxTotalSize, currentTotalSize, acceptedFormatsLabel }) => {
...
// OPRAVA: Pre initial súbory zachovávame lastModified, ak existuje
...
const FileUploadCustom = ({
  ...
  // Vyvolanie udalosti priamo v useEffect (Side Effect mimo updatovacieho cyklu)
  useEffect(() => {
  ...
  const createFileRows = (selectedFiles, currentFiles) => {
    ...
    return selectedFiles.map((file, index) => {
      ...
      return {
        ...
        name: file.name,
        size: file.size,
        lastModified: file.lastModified, // OPRAVA: Uloženie pre presnú deduplikáciu
  ...
  const handleSelectedFiles = (fileList) => {
    ...
    setFiles((currentFiles) => {
      // Fingerprint pre presnú deduplikáciu s využitím preneseného lastModified z row objektu
      ...
      const rejectedRows = rejectedSelection.map((file, index) => ({
        ...
        name: file.name,
        size: file.size,
        lastModified: file.lastModified, // OPRAVA
        status: 'error',
        progress: 0,
  ...
  const removeFile = (fileId) => {
    // Odstránenie prebehne čisto vo funkčnom update, hláška sa zavolá mimo.
    ...
    if (removedFile) {
      setActionLiveMessage(`Súbor ${removedFile.name} bol odstránený.`);
    }
  };
  ...
  return (
    <div className={cx('w-full', className)}>
      ...
      <div role="region" aria-label={regionLabel || undefined} aria-labelledby={regionLabel ? undefined : titleId} aria-describedby={describedBy}>
        ...
        <FileStatusList files={files} filesTitle={filesTitle} filesTitleId={filesTitleId} filesHeadingLevel={filesHeadingLevel} onRemove={removeFile} />
